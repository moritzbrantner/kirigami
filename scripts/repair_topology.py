...
replace_once(old_incident, new_incident, "incident edge")
...
helper_marker = '''fn orientation(a: Point2, b: Point2, c: Point2) -> f32 {
    (b.x - a.x) * (c.y - a.y) - (b.y - a.y) * (c.x - a.x)
}
'''
helper_replacement = '''fn polygon_area_tolerance(points: &[Point2]) -> f32 {
...
'''
if "fn orientation_tolerance" not in text:
    if helper_marker not in text:
        raise SystemExit("orientation helper marker not found")
    text = text.replace(helper_marker, helper_replacement, 1)

old_point_on_segment = '''fn point_on_segment(point: Point2, start: Point2, end: Point2) -> bool {
...
new_point_on_segment = '''fn point_on_segment(point: Point2, start: Point2, end: Point2) -> bool {
...
replace_once(old_point_on_segment, new_point_on_segment, "point on segment")

old_intersect = '''fn segments_intersect(a: Point2, b: Point2, c: Point2, d: Point2) -> bool {
...
new_intersect = '''fn segments_intersect(a: Point2, b: Point2, c: Point2, d: Point2) -> bool {
    let ab_c = orientation(a, b, c);
    let ab_d = orientation(a, b, d);
    let cd_a = orientation(c, d, a);
    let cd_b = orientation(c, d, b);
...
replace_once(old_intersect, new_intersect, "segment intersection")

old_triangle = '''fn point_in_or_on_triangle(point: Point2, a: Point2, b: Point2, c: Point2) -> bool {
    let ab = orientation(a, b, point);
    let bc = orientation(b, c, point);
    let ca = orientation(c, a, point);
    ab >= -EPSILON && bc >= -EPSILON && ca >= -EPSILON
}'''
new_triangle = '''fn point_in_or_on_triangle(point: Point2, a: Point2, b: Point2, c: Point2) -> bool {
    let ab = orientation(a, b, point);
    let bc = orientation(b, c, point);
    let ca = orientation(c, a, point);
...
replace_once(old_triangle, new_triangle, "triangle predicate")
...
if "fn accepts_small_non_degenerate_face" not in text:
    ...
    fn accepts_small_non_degenerate_face() {
        ...
        assert_eq!(topology.triangulate_face(FaceId(0)).unwrap().triangles.len(), 2);
